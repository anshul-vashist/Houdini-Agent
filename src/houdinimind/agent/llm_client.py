...
def load_config(config_path: str | None = None) -> dict:
    ...
    if not json_data_dir or json_data_dir == "__auto__" or not os.path.exists(json_data_dir):
        data_dir = config_dir
    # If it's a relative path, make it relative to config_dir
    elif not os.path.isabs(json_data_dir):
...
class OllamaClient:
    ...
    def apply_runtime_config(self, config: dict):
        ...
        self._context_window_resolved: int | None = None
        ...
        if data_dir and data_dir != "__auto__":
            ...
            self._embed_cache.load_from_disk(self._embed_cache_path, embed_model=self.embed_model)
        else:
    ...
    def _resolve_context_window(self, config_value: int) -> int:
        ...
        """
        try:
            resp = self._json_request("/api/show", payload={"model": self.model}, timeout=10)
    ...
    # Model selection
    # ------------------------------------------------------------------
    def _get_model_for(self, task: str | None = None) -> str:
        """
        Model selection:
    ...
    def chat_with_image(
        ...
        b64 = (
            base64.b64encode(image_bytes).decode("utf-8") if image_bytes is not None else image_b64
        )
        if b64 is None:
        ...
        def _do():
            return (
                json.loads(self._json_request("/api/chat", payload=payload, timeout=180))
                .get("message", {})
                .get("content", "")
            )
    ...
    # Retry wrapper
    # ------------------------------------------------------------------
    def _request_with_retry(self, req_fn, retries: int | None = None):
    ...
    def _chat_via_ollama(
        ...
        try:
            ...
            if e.code == 500:
                raise ConnectionError(
                    "HTTP 500: Internal Server Error. The LLM backend (Ollama/Kimi) crashed. "
    ...
    def chat_vision(
        ...
        return self._chat_vision_via_ollama(prompt, image_bytes=image_bytes, image_b64=image_b64)
    ...
    def flush_embed_cache(self) -> None:
        ...
        if getattr(self._embed_cache, "_dirty", False):
            try:
                self._embed_cache.save_to_disk(self._embed_cache_path, embed_model=self.embed_model)
            except Exception:
                pass
