...
def _check_required_strings(tool_name: str, args: dict) -> PreconditionResult:
    ...
    if tool_name == "connect_nodes":
        endpoint_pairs = (("from_path", "from_node"), ("to_path", "to_node"))
        for canonical, legacy in endpoint_pairs:
            if not (_coerce_path(args.get(canonical)) or _coerce_path(args.get(legacy))):
                return PreconditionResult.block(
                    f"Precondition: connect_nodes requires non-empty {canonical}.",
                    suggested_fix="Provide both from_path and to_path as full paths.",
                )
