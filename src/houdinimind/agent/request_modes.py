...
# Catch "convert to HDA" / "make an HDA" / "package as digital asset" etc.
# These always require write tools regardless of the leading verb.
HDA_INTENT_RE = re.compile(
    r"\b(hda|digital\s*asset|otl|"
    r"convert\s+.{0,40}(?:to|into)\s+hda|"
    r"(?:make|create|build|package)\s+.{0,40}(?:hda|digital\s*asset))\b",
    re.IGNORECASE,
)
...
class AutoResearcher:
    ...
    def run(self, query: str, progress_callback: Callable[[str], None] | None = None) -> list[dict]:
        def _p(msg):
        ...
        for iteration in range(self.max_iterations):
            ...
            if iteration < self.max_iterations - 1 and len(all_context) >= 2:
                _p("\u200b\U0001f9d0 Checking for critical gaps…")
    ...
    def select_best_option(
        ...
        for idx, option in enumerate(options):
            ...
            if self._contains_any(text, ["null", "output", "display flag", "render flag"]):
                score += 2.5
            ...
            if request_mode in {"build", "debug"}:
                if self._contains_any(text, ["quick", "simple", "basic", "primitive", "blockout"]):
                    score += 2.0
            ...
            if self._contains_any(
                ...
                if query_wants_variation:
                    ...
                    score -= 2.2
            if self._contains_any(text, ["material", "shader", "shading", "shop_materialpath"]):
