...
class HoudiniMindPanel(
    ...
    def __init__(self, parent=None):
        ...
        # Pre-resolve every hou.* read we need during init on the main
        # thread. The HOM is not thread-safe; calling hou.* from the
        # daemon below would acquire an internal HOM lock that serializes
        # against the main thread's cook/redraw, freezing the Houdini UI
        # for the whole duration of init.
        self._hou_job_dir = ""
        self._hou_app_version = "N/A"
        try:
            import hou as _hou

            try:
                job = _hou.expandString("$JOB") or ""
                if job and job != "$JOB":
                    self._hou_job_dir = job
            except Exception:
                pass
            try:
                self._hou_app_version = _hou.applicationVersionString()
            except Exception:
                pass
        except ImportError:
            pass
    ...
    def _set_init_loading_state(self, loading: bool) -> None:
        ...
        if loading:
            ...
            if inp:
                # Keep the text box ENABLED so Houdini's Qt event loop keeps
                # processing input events. Disabling it held the focus/repaint
                # cycle and made the whole UI feel frozen until init finished.
                # The send button is already disabled — submitting early shows
                # a "still initializing" note via _send().
                inp.setEnabled(True)
                inp.setPlaceholderText(
                    "HoudiniMind is initializing — send is disabled until ready…"
                )
        else:
            if send:
    ...
    def _on_backend_ready(self, error_msg: str) -> None:
        ...
        if error_msg:
            ...
            try:
                ...
                pass
            # If schema was missing, generate it now on the main thread (safe for
            # hou.*). Delay 2 s so the panel finishes rendering before the
            # potentially slow schema walk begins.
            pending = getattr(self, "_schema_path_pending", None)
            if pending:
                QtCore.QTimer.singleShot(
                    2000,
                    lambda p=pending: self._generate_schema_on_main_thread(p),
                )
