...
class PanelWorkflowMixin:
    def _inject_scene(self, silent: bool = False):
        if self._busy and not silent:
            self._add_system_note("Sync Scene is unavailable while another turn is running.")
        ...
        if not self.agent or not HOU_AVAILABLE:
            if not silent:
                self._add_system_note("Sync Scene only works inside Houdini with a live scene.")
    ...
    def _clear_conversation(self):
        ...
        self._refresh_scene_panel(overview="No scene changes yet.", summary_text="", diff_text="")
    ...
    def _run_learning_cycle(self):
        if not self.memory:
            QtWidgets.QMessageBox.information(self, "Learning", "MemoryManager not available.")
            return
    ...
    def _learn_hda(self):
        ...
        try:
            ...
            existing_titles = {e.get("title", "") for e in kb_data.get("entries", [])}
            entry_title = f"HDA Node: {node_type_name}"
            if entry_title in existing_titles:
                self._add_system_note(f"📦 {node_type_name} already in knowledge base.")
                return
    ...
    def _show_recipes(self):
        if not self.memory:
            QtWidgets.QMessageBox.information(self, "Recipes", "MemoryManager not available.")
            return
    ...
    def _start_autoresearch(self):
        if self._busy:
            self._add_system_note("Cannot start AutoResearch while another task is running.")
            return
        if not self.agent:
        ...
        self._add_msg("user", "[Train] Starting agent training loop", "chat")
        ...
        def _run():
            try:
                self._autoresearch_loop.run(
                    ...
                    tool_callback=self.sig_tool_called.emit,
                    progress_callback=self.sig_autoresearch_progress.emit,
                )
            except Exception as e:
    ...
    def _stop_autoresearch(self):
        if self._autoresearch_loop:
            ...
            self._add_system_note("⏹ Train stop requested — finishing current task…")
            self._refresh_turn_status("Stopping training after the current task completes.")
    ...
    def _on_autoresearch_progress(self, data: dict):
        ...
        if event == "started":
            ...
            self.header_status_lbl.setText(f"Train • {data.get('lessons', 0)} lessons in memory")
            ...
            self._refresh_turn_status(f"Building {name} — attempt {attempt}/{max_attempts}")
    ...
    def _show_autoresearch_stats(self):
        try:
            ...
            QtWidgets.QMessageBox.information(self, "Training Stats", f"No data yet: {e}")
    ...
    def _last_turn_recipe_steps(self) -> list[dict]:
        steps = []
        for entry in getattr(self, "_last_turn_tools", []) or []:
            if not isinstance(entry, dict):
                continue
            tool = entry.get("name") or entry.get("tool")
            if not tool:
                continue
            steps.append({"tool": tool, "args": entry.get("args") or {}})
        if steps:
            return steps

        conversation = getattr(self.agent, "conversation", []) if self.agent else []
    ...
    def _feedback_domain(self, text: str) -> str:
        lowered = (text or "").lower()
        if any(
            kw in lowered for kw in ("chair", "table", "shelf", "lamp", "desk", "sofa", "cabinet")
        ):
            return "furniture"
        if any(kw in lowered for kw in ("pyro", "smoke", "fire", "flip", "vellum", "rbd", "sim")):
            return "fx"
        return "general"

    def _record_positive_recipe(self):
        """Immediately save a thumbs-up turn as a reusable recipe."""
        try:
            if not self.memory:
                return
            steps = self._last_turn_recipe_steps()
            if not steps:
                return
            last_msg = (
                getattr(self.memory, "_last_user_message", "")
                or getattr(self, "_last_user_request", "")
                or "unknown"
            )
            domain = self._feedback_domain(last_msg)
            tool_summary = " → ".join(s["tool"] for s in steps[:8])
            name = f"accepted_{hash((last_msg[:160], tool_summary)) & 0xFFFFFF:06x}"
            recipe_id = self.memory.recipes.add_recipe(
                name=name,
                description=(
                    f"Accepted approach for: '{last_msg[:120]}'. Tools used: {tool_summary}"
                ),
                trigger_pattern=last_msg[:220],
                steps=steps,
                domain=domain,
            )
            if recipe_id > 0:
                self.memory.recipes.record_use(recipe_id, accepted=True, complexity_weight=1.5)
                print(f"[HoudiniMind] Positive recipe recorded: {name} ({len(steps)} steps)")
        except Exception as e:
            print(f"[HoudiniMind] _record_positive_recipe error: {e}")
    ...
    def _record_negative_recipe(self):
        ...
        try:
            if not self.agent or not self.memory:
                return
            tool_steps = self._last_turn_recipe_steps()
            if not tool_steps:
            ...
            self.memory.recipes.add_negative_recipe(
                name=name,
                description=(
                    f"Rejected approach for: '{last_msg[:120]}'. Tools used: {tool_summary}"
                ),
    ...
    def _set_busy(self, busy: bool):
        ...
        elif self._autoresearch_running:
            self.composer_hint_lbl.setText("Training running… click Train to stop")
        else:
    ...
    def _show_debug_log(self):
        if not self.agent or not getattr(self.agent, "debug_logger", None):
            ...
            self._refresh_turn_status("Run a turn first to generate a session debug log.")
            return
        try:
            log_path = self.agent.debug_logger.get_session_path()
        except Exception as exc:
            self._add_system_note(f"Debug log unavailable: {exc}")
            self._refresh_turn_status("The current session debug log could not be opened.")
            return
        if not log_path or not os.path.isfile(log_path):
            self._add_system_note("No session debug log file was found for the current run.")
            self._refresh_turn_status("Run a turn first to generate a session debug log.")
            return
        ...
        dialog.destroyed.connect(lambda *_args: setattr(self, "_debug_log_dialog", None))
