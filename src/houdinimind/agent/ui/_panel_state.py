...
from ._widgets import ModernStyles
...
class PanelStateMixin:
    ...
    def _begin_turn_ui(self, user_text: str, mode: str, dry_run: bool = False):
        ...
        self._refresh_turn_status("Mapping out the request and preparing the next action.")
    ...
    def _refresh_turn_status(self, latest_message: str = ""):
        ...
        if latest_message:
            ...
            self.turn_status_lbl.setText(self._elide_text(self.turn_status_lbl, latest_message))
        ...
        failure_count = len(self._current_turn_failures if self._busy else self._last_turn_failures)
        elapsed = (
        ...
        if self._current_dry_run or (
            self.agent and getattr(self.agent, "_last_turn_dry_run", False) and not self._busy
        ):
            meta += " • Dry Run"
    ...
    def _refresh_tools_panel(self, reset: bool = False):
        ...
        latest = self._current_turn_tools[-1]["name"] if self._current_turn_tools else ""
        if tool_count == 0:
            summary = "Tool activity for the current turn will appear here."
        else:
            summary = f"{tool_count} step{'s' if tool_count != 1 else ''}"
            if failure_count:
                summary += f" • {failure_count} issue{'s' if failure_count != 1 else ''}"
            if latest:
    ...
    def _refresh_scene_panel(
        ...
        failures = self._current_turn_failures if self._busy else self._last_turn_failures
        review = "Needs review" if failures else "No warnings"
        if self._busy and not overview:
            overview = "Working through the build and keeping the final output on track."
        ...
        if self.agent and not self._busy:
            checkpoint_text = getattr(self.agent, "_last_turn_checkpoint_path", "") or "—"
            verification_text = getattr(self.agent, "_last_turn_verification_text", "") or "—"
        mode_label = {
            "chat": "Chat",
            "vision": "Vision",
            "research": "Research",
        ...
        try:
            ...
            if recipes:
                ...
                for recipe in recipes:
                    name = recipe.get("name") or recipe.get("title") or "Untitled recipe"
