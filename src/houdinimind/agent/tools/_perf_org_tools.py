...
def audit_spatial_layout(parent_path):
    ...
    try:
            ...
            audit.append(
                {
                    ...
                    "at_origin": (abs(tx) < 0.001 and abs(ty) < 0.001 and abs(tz) < 0.001),
        ...
        non_structural = [a for a in audit if a["type"] not in _STRUCTURAL_TYPES]
        ...
        geometry_items = [a for a in non_structural if isinstance(a.get("bbox"), dict)]
...
def find_and_replace_parameter(root_path, search_value, replace_value, parm_name_filter=""):
...
def beautify_network(parent_path, box_name="Setup_Network"):
    """Align nodes and color-code them by semantic category for professional organization."""
    try:
        _require_hou()
        parent = hou.node(parent_path)
        if not parent:
        ...
        if not nodes:
            return _ok(message="No nodes to beautify.")

        # 1. Color Coding Logic
        color_map = {
            "generator": hou.Color((0.3, 0.5, 1.0)),  # Blue
            "simulation": hou.Color((0.3, 0.8, 0.3)),  # Green
            "render": hou.Color((1.0, 0.3, 0.3)),  # Red
            "logic": hou.Color((1.0, 0.8, 0.1)),  # Yellow
            "lookdev": hou.Color((0.8, 0.4, 1.0)),  # Purple
            "default": hou.Color((0.6, 0.6, 0.6)),  # Gray
        }
        ...
        category_map = {
            ...
            "flip": "simulation",
            "dop": "simulation",
            "karmamaterial": "lookdev",
            "principledshader": "lookdev",
            "usd": "lookdev",
            "output": "render",
            "rop": "render",
        }
        ...
        # 3. Network Box
        try:
            from ._chain_tools import create_network_box

            create_network_box(parent_path, [n.path() for n in nodes], box_name)
        except Exception:
            pass

        return _ok(message=f"Beautified {len(nodes)} nodes in {parent_path}")
    except Exception as e:
        return _err(str(e))
...
def set_keyframe(node_path, parm_name, value, frame=None, slope_in=None, slope_out=None):
