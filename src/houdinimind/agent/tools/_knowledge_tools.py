...
def search_knowledge(query: str, top_k: int = 5, category_filter: str | None = None) -> dict:
    ...
    try:
        ...
        if retriever:
            ...
            return core._ok({"query": query, "results_found": len(formatted), "results": formatted})
    except Exception:
        pass
    return core._lexical_search_knowledge(query, top_k=top_k, category_filter=category_filter)
...
def get_error_fix(error_message: str) -> dict:
    ...
    broad_hits = [
        r for r in (broad["data"]["results"] if broad["status"] == "ok" else []) if r not in fixes
    ]
    return core._ok(
        {
