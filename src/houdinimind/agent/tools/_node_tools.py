...
_DETAIL_WRANGLE_FUNCTION_RE = re.compile(
    r"\b(?:addpoint|addprim|addvertex|removepoint|removeprim|removevertex)\s*\(",
    re.I,
)
_DETAIL_WRANGLE_ITERATION_RE = re.compile(
    r"\bfor\s*\([^;]*;[^;]*\b(?:npoints|nprimitives|nvertices)\s*\(\s*0\s*\)",
    re.I | re.S,
)


def _strip_vex_comments_and_strings(vex_code):
    """Remove strings/comments so function-name scans do not trigger on prose."""
    if not isinstance(vex_code, str):
        return ""
    return re.sub(
        r"//[^\n]*|/\*.*?\*/|'(?:\\.|[^'\\])*'|\"(?:\\.|[^\"\\])*\"",
        "",
...
def _vex_requires_detail_wrangle(vex_code):
    stripped = _strip_vex_comments_and_strings(vex_code)
    return bool(
        _DETAIL_WRANGLE_FUNCTION_RE.search(stripped)
        or _DETAIL_WRANGLE_ITERATION_RE.search(stripped)
    )


def _infer_wrangle_class_for_vex(node, vex_code):
    try:
        node_type_name = str(node.type().name()).lower()
        if node_type_name.split("::", 1)[0] != "attribwrangle":
            return None
    except Exception:
        return None
    if _vex_requires_detail_wrangle(vex_code):
        return "detail"
    return None


def _parm_eval_normalized(parm):
    try:
        value = parm.eval()
    except Exception:
        return None
...
def _ensure_wrangle_class_for_vex(node, vex_code):
    inferred_class = _infer_wrangle_class_for_vex(node, vex_code)
    if inferred_class != "detail":
        return None
    class_parm = node.parm("class")
    if not class_parm:
        return {
            "status": "error",
            "message": f"VEX requires a detail-class wrangle, but {node.path()}/class was not found.",
        }
    old_class = _snapshot_parm_value(class_parm)
    if _parm_eval_normalized(class_parm) in {"detail", "3"}:
        return {"status": "ok", "class": "detail", "old": old_class, "changed": False}
    set_result = _set_parm_value(class_parm, "detail")
    if set_result.get("success") is False:
        return {
            "status": "error",
            "message": (
                f"VEX requires a detail-class wrangle, but setting "
                f"{node.path()}/class to detail failed: {set_result.get('error')}"
            ),
        }
    if _parm_eval_normalized(class_parm) not in {"detail", "3"}:
        return {
            "status": "error",
            "message": (
                f"VEX requires a detail-class wrangle, but {node.path()}/class "
                f"did not read back as detail after setting it."
            ),
        }
    return {"status": "ok", "class": "detail", "old": old_class, "changed": True}
...
def _set_node_parameter(node, parm_name, value, parm_cache=None, cook_vex=True):
    ...
    if resolved.get("status") == "resolved":
        ...
        # Fallback to LLM fuzzy matching (Auto-RAG) using the live parameter pool
        fuzzy_match = core._fuzzy_match_parameter(
            parm_name,
            actual_parm_names,
            labels_by_name,
            node_type=node_type,
        )
        if fuzzy_match and fuzzy_match in actual_parm_names:
            parm_name = fuzzy_match
            lowered_parm_name = parm_name.lower()
    ...
    if parm_name == "snippet" and isinstance(value, str):
        ...
        wrangle_class_result = _ensure_wrangle_class_for_vex(node, value)
        if wrangle_class_result and wrangle_class_result.get("status") != "ok":
            return _err(wrangle_class_result.get("message", "Wrangle class inference failed"))
    ...
    if parm_name == "snippet" and isinstance(value, str):
        ...
        if cook_errors:
            try:
                ...
                pass
            if wrangle_class_result and wrangle_class_result.get("changed"):
                try:
                    node.parm("class").set(wrangle_class_result.get("old"))
                except Exception:
                    pass
            return _err(
        ...
        if wrangle_class_result:
            data["wrangle_class"] = wrangle_class_result.get("class")
            if wrangle_class_result.get("changed"):
                data["wrangle_class_old"] = wrangle_class_result.get("old")
        message = f"UNDO_TRACK: Set {node.path()}/snippet to validated VEX"
        if wrangle_class_result and wrangle_class_result.get("changed"):
            message += " — inferred class=detail"
        if vex_validation_unavailable:
            message += (
...
def write_vex_code(node_path, vex_code):
    ...
    try:
        ...
        wrangle_class_result = _ensure_wrangle_class_for_vex(node, vex_code)
        if wrangle_class_result and wrangle_class_result.get("status") != "ok":
            return _err(wrangle_class_result.get("message", "Wrangle class inference failed"))
        ...
        if wrangle_class_result and wrangle_class_result.get("changed"):
            message += " — inferred class=detail"
        message += (
            f" — warnings: {valid_res['warnings']}" if valid_res["warnings"] else " — compiled OK"
        )
        return _ok(data, message=message)
    except Exception as e:
        return _err(str(e))
