...
def _resolve_obj_parent(parent_path: str):
    parent = core._hou.node(parent_path)
    node = parent
    while node is not None:
        cat_name = _child_category_name(node)
        if cat_name in {"obj", "object"} or "object" in cat_name:
            return node
        try:
            node = node.parent()
        except Exception:
...
def _set_parm_if_exists(node, parm_name: str, value) -> bool:
    parm = node.parm(parm_name) if node is not None and hasattr(node, "parm") else None
    if parm:
        parm.set(value)
        return True
    return False
...
def watch_node_events(port: int = 9877, stop: bool = False, keepalive_s: int = 15) -> dict:
...
def edit_animation_curve(
    ...
    try:
        ...
        interp = None
        enum = getattr(core._hou, "keyframeInterpolation", None)
        if enum is not None:
            interp_map = {
                "constant": getattr(enum, "Constant", None),
                "linear": getattr(enum, "Linear", None),
                "bezier": getattr(enum, "Bezier", None),
                "ease": getattr(enum, "EaseInEaseOut", None),
                "easein": getattr(enum, "EaseIn", None),
                "easeout": getattr(enum, "EaseOut", None),
            }
            interp = interp_map.get(interpolation.lower())
            interpolation_supported = interp is not None and hasattr(keys[0], "setInterpolation")
        ...
        for k in keys:
            ...
            if slope_auto and hasattr(k, "setSlopeAuto"):
                k.setSlopeAuto(True)
            else:
                if in_slope is not None and hasattr(k, "setInSlope"):
                    k.setInSlope(in_slope)
                if out_slope is not None:
                    if hasattr(k, "setOutSlope"):
                        k.setOutSlope(out_slope)
                    elif hasattr(k, "setSlope"):
                        k.setSlope(out_slope)
        parm.setKeyframes(keys)
        data = {
            "parm": parm_name,
            "keyframe_count": len(keys),
            "interpolation": interpolation,
            "interpolation_supported": interpolation_supported,
        }
        message = "Animation curve updated."
        if not interpolation_supported:
            message += " Houdini did not expose keyframe interpolation enums; slope settings were applied where available."
        return core._ok(data, message=message)
    except Exception:
        return core._err(core._traceback.format_exc())
...
def setup_wire_solver(
    ...
    core._require_hou()
    parent = _resolve_obj_parent(parent_path)
    if not parent:
        return core._err(f"Parent not found: {parent_path}")
    try:
        source_obj = parent.createNode("geo", "wire_setup")
        om = source_obj.createNode("object_merge", "source_curves")
        _set_parm_if_exists(om, "objpath1", geo_node_path)
        _set_parm_if_exists(om, "xformtype", 1)
        dopnet = parent.createNode("dopnet", "wire_sim")
        _set_parm_if_exists(dopnet, "gravity", gravity)
        wire_dop = dopnet.createNode("wireobject", "wire_dop")
        _set_parm_if_exists(wire_dop, "soppath", om.path())
        solver = dopnet.createNode("wiresolver", "wire_solver")
        _set_parm_if_exists(solver, "stiffness", stiffness)
        _set_parm_if_exists(solver, "dampingratio", damping)
        ...
        return core._ok(
            {
                "source_geo": source_obj.path(),
                "wire_object": wire_dop.path(),
                "dopnet": dopnet.path(),
                "solver": solver.path(),
            }
        )
...
def setup_grain_sim(
    ...
    try:
        ...
        _set_parm_if_exists(om, "objpath1", source_node_path)
        _set_parm_if_exists(om, "xformtype", 1)
        scatter = geo_obj.createNode("scatter", "scatter_grains")
        _set_parm_if_exists(scatter, "npts", 5000)
        ...
        pop_object = dopnet.createNode("popobject", "grain_object")
        pop_grains = dopnet.createNode("popgrains", "pop_grains")
        _set_parm_if_exists(pop_grains, "friction", friction)
        _set_parm_if_exists(pop_grains, "clumping", clumping)
        pop_source = dopnet.createNode("popsource", "grain_source")
        _set_parm_if_exists(pop_source, "soppath", add_attrib.path())
        solver = dopnet.createNode("popsolver", "pop_solver")
        solver.setInput(0, pop_object)
        solver.setInput(1, pop_source)
        solver.setInput(2, pop_grains)
        out = dopnet.createNode("output", "out")
        out.setInput(0, solver)
        return core._ok(
            {
                "geo_setup": geo_obj.path(),
                "dopnet": dopnet.path(),
                "pop_object": pop_object.path(),
                "pop_source": pop_source.path(),
                "pop_grains": pop_grains.path(),
                "solver": solver.path(),
            }
        )
    except Exception:
...
def set_viewport_display_mode(mode: str, pane_index: int = 0) -> dict:
    ...
    candidates = {
        ...
        "wireghost": ("WireGhost",),
        "flat": ("Flat",),
        "hidden_invis": ("HiddenLineInvisible",),
        "points": ("Points", "Wire"),
    }
    shading = None
    for attr in candidates.get(mode.lower(), ()):
        shading = getattr(gl, attr, None)
    ...
    try:
        ...
        if hasattr(settings, "setShadingMode"):
            settings.setShadingMode(shading)
        elif hasattr(settings, "displaySet"):
            display_type = getattr(core._hou.displaySetType, "CurrentModel", None)
            display_set = settings.displaySet(display_type) if display_type is not None else None
            if display_set and hasattr(display_set, "setShadedMode"):
                display_set.setShadedMode(shading)
            else:
                return core._err("Viewport display set does not support shaded mode edits.")
        else:
            return core._err("Viewport settings do not support shaded mode edits.")
