...
def _include_project_source_fallbacks(data_dir: str | None = None) -> bool:
    if not data_dir:
        return True
    return os.path.abspath(data_dir) == os.path.abspath(DATA_DIR)


def _cwd_source_fallbacks() -> list[str]:
    if os.path.abspath(os.getcwd()) != os.path.abspath(ROOT):
        return []
    return [os.getcwd(), os.path.dirname(os.getcwd())]
...
def _houdini_python_source_candidates(data_dir: str | None = None) -> list[str]:
    ...
    data_roots = [data_dir]
    if _include_project_source_fallbacks(data_dir):
        data_roots.append(DATA_DIR)
    ...
    if _include_project_source_fallbacks(data_dir):
        candidates.append(os.path.join(ROOT, "houdini_python_functions.json"))
        for root in _cwd_source_fallbacks():
            candidates.append(os.path.join(root, "houdini_python_functions.json"))
...
def _vex_db_source_candidates(data_dir: str | None = None) -> list[str]:
    ...
    if _include_project_source_fallbacks(data_dir):
        candidates.append(os.path.join(ROOT, "vex_functions.db"))
        for root in _cwd_source_fallbacks():
            candidates.append(os.path.join(root, "vex_functions.db"))
...
def _vex_jsonl_source_candidates(data_dir: str | None = None) -> list[str]:
    candidates = []

    env_value = os.environ.get("HOUDINIMIND_VEX_JSONL_PATH", "").strip()
    if env_value:
        for part in env_value.split(os.pathsep):
            part = part.strip()
            if part:
    ...
    for root in data_roots:
        ...
        if os.path.exists(knowledge_dir):
            for pattern in (
                ...
                candidates.extend(glob.glob(os.path.join(knowledge_dir, pattern)))
        candidates.extend(
            [
                os.path.join(root, "vex_dataset_final_merged.jsonl"),
                os.path.join(os.path.dirname(root), "vex_dataset_final_merged.jsonl"),
            ]
        )

    if _include_project_source_fallbacks(data_dir):
        candidates.append(os.path.join(ROOT, "vex_dataset_final_merged.jsonl"))
        for root in _cwd_source_fallbacks():
            candidates.append(os.path.join(root, "vex_dataset_final_merged.jsonl"))
...
def _strip_reasoning_channel(text: str) -> str:
    return _THOUGHT_CHANNEL_RE.sub("", str(text or "")).strip()


def _normalise_vex_code_block(code: str) -> str:
    lines = []
    for line in str(code or "").strip().splitlines():
        stripped = line.strip()
        if stripped.lower() in {"### vex implementation", "# vex implementation"}:
            continue
        lines.append(line.rstrip())
    return "\n".join(lines).strip()


def _vex_jsonl_record_to_entry(record: dict, source_path: str, index: int) -> dict:
    messages = record.get("messages") or []
    if not messages and record.get("contents"):
        messages = _gemini_contents_to_messages(record.get("contents"))
    prompt, answer_text = _split_prompt_and_code(messages)
    answer_text = _strip_reasoning_channel(answer_text)
    code = _normalise_vex_code_block(_extract_fenced_code(answer_text))

    prompt_clean = str(prompt or "").strip()
    if not prompt_clean and not code:
        return {}

    title = prompt_clean or f"VEX Dataset Example {index}"
    ...
    tags = _dedupe_tags(
        [
            ...
            *_slug_tokens(code[:500]),
        ]
    )

    content_parts = [f"Task: {prompt_clean or 'unspecified'}"]
    if code:
        content_parts.append("VEX:\n" + code)

    return {
        "title": f"VEX Dataset Example: {title[:120]}",
        "category": "vex",
        "tags": tags,
        "content": "\n".join(content_parts),
        "_source": f"{source_stem}_jsonl",
        "_source_path": os.path.basename(source_path),
        "_record_index": index,
    }


def _load_vex_jsonl_knowledge(data_dir: str | None = None) -> list:
    """Load VEX JSONL examples as retrieval-grounding examples, stripped of reasoning."""
    entries = []
    loaded_paths = []
    seen_prompts = set()

    for source_path in _vex_jsonl_source_candidates(data_dir):
        if not os.path.exists(source_path):
            continue
        loaded = 0
        try:
            with open(source_path, encoding="utf-8") as f:
                for index, line in enumerate(f, start=1):
                    ...
                    if not isinstance(record, dict):
                        continue
                    entry = _vex_jsonl_record_to_entry(record, source_path, index)
                    if not entry:
                        continue
            ...
            print(f"[KB Builder] Failed to read VEX JSONL {source_path}: {e}")
            continue

        if loaded:
            loaded_paths.append(source_path)
            print(
                f"[KB Builder] Loaded {loaded} VEX JSONL knowledge entries from "
                f"{os.path.basename(source_path)}"
            )

    if loaded_paths:
        print(
            f"[KB Builder] Loaded {len(entries)} total VEX JSONL knowledge entries "
            f"from {len(loaded_paths)} source file(s)"
        )
    return entries
...
def build_kb(output_path: str | None = None, verbose: bool = True) -> str:
    ...
    all_entries.extend(_load_vex_jsonl_knowledge())
