...
class ContextInjector:
    ...
    def _estimate_query_complexity(self, query: str) -> str:
        ...
        if word_count < 8 and not tech_matches:
            if re.search(
                r"\b(create|build|make|design|construct|generate|model|sculpt)\b",
                query,
                re.IGNORECASE,
            ):
                return "medium"
    ...
    def _enrich_query(self, query: str, request_mode: str | None) -> list[str]:
        """
    ...
    def build_context_message(
        ...
        force_chunks: list[str] | None = None,  # chunk IDs to always include
        ...
        has_workflow_chunk = any(str(c.get("category", "")).lower() == "workflow" for c in chunks)
        ...
        needs_workflow = str(request_mode or "").lower() == "build" and not has_workflow_chunk
        ...
        if not chunks or weak_primary or needs_workflow:
            ...
            for fallback_query in fallback_queries:
                ...
                if len(chunks) >= effective_top_k and (not needs_workflow or has_workflow_chunk):
                    break
        ...
        if (
            ...
            second_score = (
                float(_non_scene_chunks[1].get("_score", 0)) if len(_non_scene_chunks) > 1 else 0.0
            )
        ...
        chunk_previews = [c.get("content", "")[:120].replace("\n", " ") for c in chunks[:10]]
        self.last_context_meta = {
            "query": query,
            ...
            "budget_pct_used": round(estimated_tokens / effective_budget * 100, 1)
            if effective_budget
            else None,
    ...
    def _format_chunks(self, chunks: list[dict], query: str, max_tokens: int | None = None) -> str:
        """
